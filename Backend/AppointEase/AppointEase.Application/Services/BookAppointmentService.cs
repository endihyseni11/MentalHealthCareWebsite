...
namespace AppointEase.Application.Services
{
    public class BookAppointmentService : IBookAppointmentService
    {
        ...
        public BookAppointmentService(IMapper mapper, IApplicationExtensions common, IOperationResult operationResult, IRepository<BookAppointment> bookappointmentReposiotry, IRepository<AppointmentSlot> appointmentSlotRepository, IStripeApi stripeApi)
        {
            _mapper = mapper;
            _common = common;
        ...
        public async Task<OperationResult> CreateBookAppointment(BookAppointmentRequest bookappointment)
        {
            try
            {
                ...
                var paymentIntentRequest = new PaymentIntentRequest // Adjust namespace
                {
                    Amount = bookappointment.paymentIntentRequest.Amount, // Example amount, adjust according to your logic
                    Currency = bookappointment.paymentIntentRequest.Currency, // Example currency, adjust according to your logic
                    PaymentMethodTypes = bookappointment.paymentIntentRequest.PaymentMethodTypes, // Example payment method types, adjust according to your logic
                    PatientId = bookappointment.paymentIntentRequest.PatientId, // Example patient ID, adjust according to your logic
                    PaymentMethod = bookappointment.paymentIntentRequest.PaymentMethod // Example payment method, adjust according to your logic
                };

                var clientSecret = await _stripeApi.CreatePaymentIntent(paymentIntentRequest);
